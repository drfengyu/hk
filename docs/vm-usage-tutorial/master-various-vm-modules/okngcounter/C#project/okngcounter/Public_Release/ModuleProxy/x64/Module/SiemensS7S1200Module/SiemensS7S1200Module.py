...
def config_logging():
    # format
     fmt = logging.Formatter(fmt='SiemensS7S1200Module %(asctime)s.%(msecs)03d [%(thread)5s] %(levelname)-8s %(funcName)-20s %(lineno)d %(message)s', datefmt='%Y:%m:%d %H:%M:%S')
...
def HandShake(paramlist):#如果需要註冊，則填充註冊请求命令,如果不需要，直接返回空即可
    ...
    Register += b'\xC0' #Parameter Code (src-tasp) 参数代码TPDU-Size
    Register += b'\x01' #Parameter Length 参数长度

    Register += b'\x0A' #Source TASP S71200 TPDU大小
    Register += b'\xC1' #Parameter Code (dst-tasp) 参数代码SRC-TASP
    Register += b'\x02' #参数长度


    Register += b'\x01' #Parameter Length
    Register += b'\x02' #Destination TASP   
    Register += b'\xC2' #Parameter Code (tpdu-size)
    Register += b'\x02' #Parameter Length
    Register += b'\x01' #TPDU Size (2^10 = 1024)
    nTasp = (nRack << 5) | nSlot
    Register += nTasp.to_bytes(1,byteorder = 'little') #Destination TASP
    global bRegister
    bRegister = False
...
def HandShakeSecond(paramlist):#如果需要註冊，則填充註冊请求命令,如果不需要，直接返回空即可
    ...
    Register += b'\x01\x00\x00\x04\x00\x00\x08\x00'
    Register += b'\x00\xF0\x00\x00\x01\x00\x01\x01\xE0'
